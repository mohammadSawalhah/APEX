  ...
  CREATE OR REPLACE EDITIONABLE PACKAGE "MOBASHER"."AWS4_S3_PKG2" as
...
/
CREATE OR REPLACE EDITIONABLE PACKAGE BODY "MOBASHER"."AWS4_S3_PKG2" as
-----------------------------------------------------------------------------------------
--
...
-- the following global settings will need to be changed for your environment
  ...
  g_gmt_offset			number 								:= 0; -- your timezone GMT adjustment
	g_aws_region			varchar2(40) 					:= 'ap-south-1';
	g_aws_service			varchar2(40) 					:= 's3';
	-- this information appears within the XML data that returns. 
  g_aws_namespace_s3			constant varchar2(255):= 'https://s3.amazonaws.com/doc/2006-03-01/';
...
begin
	l_url := aws4_s3_pkg2.get_object_url (
		P_BUCKET 				=> P_BUCKET,
		P_CANONICAL_URI	=> P_CANONICAL_URI,
		P_DATE					=> sysdate
		);
